...
> **Статус:** закрыт полностью 2026-09-24 — в архив (Р-2 бэклога `развитие-приложения`).
> Блоки 1–6 🏁, прод на Go + Vue с 2026-09-24. Хвосты: баг «Еда и быт», офлайн-бейдж, Н-6, Н-7 → Блок 0 `развитие-приложения`; «Уборка старого» — отдельная S-задача не раньше 2026-10-08; остальные — в `memory/STATE.md`.
...
| Хвост | Откуда | Судьба |
|---|---|---|
| Вход через Google OAuth и Apple ID | Запрос владельца | ➡️ `развитие-приложения`: после MVP (Р-1 — после домена) |
| Биометрия / Face ID / Touch ID (Passkeys / WebAuthn) | Запрос владельца | ➡️ `развитие-приложения`: после MVP (Р-1 — после домена) |
| Демо-режим (песочница без регистрации для ознакомления) | Запрос владельца | ✅ сделан в Блоке 3 (экран `Access.vue`) |
| Публичный лэндинг с информацией о приложении | Запрос владельца | ➡️ `memory/STATE.md` «Хвосты без судьбы» |
...
| Поддержка мульти-домохозяйств и переключение активной семьи | Ревью backend (Н-11) | ➡️ `memory/STATE.md` «Хвосты без судьбы» |
...
| `data` синка с `\u0000` или одиночным суррогатом (`\udc00`): `jsonb` отвергает → 500, синк клиента застревает в `error` (битый UTF-8 уже → 400) | Handoff исполнителя Блока 6, критик | ➡️ `memory/STATE.md` «Хвосты без судьбы» |
...
| Офлайн шапка Vue показывает «синхронизировано» — индикатор не слушает сеть до попытки синка | Handoff исполнителя Блока 6 | ➡️ `развитие-приложения` RP-04 |
...
| **Уборка старого**: удаление React `src/` (и корневых `vite.config.ts`/`package.json` React), таблиц React в `public`, пользователей Supabase Auth, Edge Function `fx-rate`, env Preview ветки; заодно `frontend/e2e/pwa-build.test.ts` — сравнение иконки с `../../public/favicon.svg` заменить проверкой содержимого (`REVIEW-frontend.md` Н-5) | Бриф Блока 6; ревью frontend Н-5 | ➡️ отдельная S-задача не раньше 2026-10-08 и с «да» владельца (`развитие-приложения` Р-2); строка в `memory/STATE.md` |
| **«Еда и быт» перебивается партнёром**: `NumFieldBlur` коммитит на каждый blur без изменений со свежим `updatedAt` — устройство со старым значением тапом по полю затирает правку партнёра (LWW); унаследовано от React `kit.tsx` | Смоук cutover (владелец: «слетел опять») | ➡️ `развитие-приложения` RP-01 |
| `pullHousehold` после повторного входа затирает неотправленное при `error`; сначала `logout` сбрасывает стор `finance`, потом слияние при любом не-`idle` | `REVIEW-frontend.md` Н-6 | ➡️ `развитие-приложения` RP-04 |
| Демо-режим: правка → `scheduleSync` → 401 → бейдж «не сошлось»; решить, какой статус показывать в демо, и не ходить на сервер | `REVIEW-frontend.md` Н-7 (клинап не закрыл — продуктовый вопрос) | ➡️ `развитие-приложения` RP-05 |
