...
> **Статус:** Блоки 2–5 🏁; Блок 1 — код ✅, 🏁 снят повторным клинапом 2026-09-24: деплоя не было, коммиты не запушены (CI не запускался), SQL-слой проверялся только на моках (исправлено — тесты на реальном PostgreSQL).
> Следующий шаг: владелец — `git push` (CI зелёный) и решение по прод-инфраструктуре Go + переносу данных из Supabase (хвосты ниже); затем деплой → 🏁 Блока 1 → архив.
