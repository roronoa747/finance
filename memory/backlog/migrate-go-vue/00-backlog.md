...
> **Статус:** Блоки 2–5 🏁; Блок 1 — код ✅, 🏁 снят повторным клинапом 2026-09-24: деплоя не было, CI не запускался (`main` намеренно не пушится, чтобы не задеть прод на Vite), SQL-слой проверялся только на моках (исправлено — тесты на реальном PostgreSQL).
> Следующий шаг: владелец — push в отдельную ветку, не в `main` (CI зелёный) и решение по прод-инфраструктуре Go + переносу данных из Supabase (хвосты ниже); затем деплой → 🏁 Блока 1 → архив.
...
|---|---|---|---|---|
| **1. Go Backend** ✅ принят · 🏁 после деплоя *(L: безопасность, БД)* | MGV-01 Скелет Go-сервера, конфиг и подключение к PostgreSQL | `block-1-backend/MGV-01-go-skeleton-db.md` | — | ✅ |
...
| **Блокер деплоя**: fail-fast при `APP_ENV=production` без `DATABASE_URL` (иначе молча in-memory моки) и с дефолтным `JWT_SECRET` (подделка токенов) | Повторный клинап Блока 1 | ⏳ судьбу назначает владелец |
| Прод-инфраструктура Go (хостинг, прод-PostgreSQL, домен, env) + **перенос данных из Supabase** — задач в бэклоге нет | Повторный клинап Блока 1 | ⏳ судьбу назначает владелец |
| Коды ошибок синка: битый UTF-8 → 500 (надо 400), тело > лимита → 400 (надо 413) | Повторный клинап Блока 1 | ⏳ судьбу назначает владелец |
