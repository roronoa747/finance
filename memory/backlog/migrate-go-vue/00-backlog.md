...
> **Статус:** Блоки 1–5 🏁 (у Блока 1 деплой перенесён в Блок 6 решением владельца 2026-09-24; CI бэкенда зелёный на `mgv-ci`). Блок 6 «Прод-инфраструктура и cutover» ⬜ — задач ещё нет.
> Следующий шаг: `/brief` по Блоку 6 (хостинг Go, прод-PostgreSQL, домен, env, fail-fast, перенос данных из Supabase, cutover) → `/backlog` дописывает задачи Блока 6.
...
| **6. Прод-инфраструктура и cutover** ⬜ *(решение владельца 2026-09-24; задачи строит `/brief` → `/backlog`)* | Деплой Go + Vue одним выходом, fail-fast прода, перенос данных из Supabase, смоук, план отката. **Инфраструктура — та же (владелец 2026-09-24): Vue и Go (Vercel-функция) на Vercel, PostgreSQL — Supabase.** Открыто для брифа: адаптер Go под Vercel, миграции отдельным шагом, пулер Supavisor; пересечение имён таблиц Go с живыми таблицами Supabase (`IF NOT EXISTS` молча пропустит — нужна своя схема/проект); перенос пользователей из `auth.users` (bcrypt) | — | Блоки 1–5 | ⬜ |
...
| **Блокер деплоя**: fail-fast при `APP_ENV=production` без `DATABASE_URL` (иначе молча in-memory моки) и с дефолтным `JWT_SECRET` (подделка токенов) | Повторный клинап Блока 1 | ➡️ Блок 6 (предусловие деплоя) |
| Прод-инфраструктура Go (хостинг, прод-PostgreSQL, домен, env) + **перенос данных из Supabase** — задач в бэклоге нет | Повторный клинап Блока 1 | ➡️ Блок 6 (решение владельца 2026-09-24) |
| Коды ошибок синка: битый UTF-8 → 500 (надо 400), тело > лимита → 400 (надо 413) | Повторный клинап Блока 1 | ➡️ кандидат в Блок 6 (решит бриф Блока 6) |
