...
import { Card, Callout, Row, Section, Segmented, Stat, Tag } from '@/components/kit'
...
import { amountAt, liveCredits, liveObligations, totalIncome, useStore } from '@/store/useStore'
              ...
              {categories.filter((c) => c.key !== 'd5').map((c) => (
                <div key={c.key} className="flex items-center gap-3 border-b border-line py-3 last:border-b-0">
                  ...
                  <div className="min-w-0 flex-1">
                    ...
                    <div className="text-[12.5px] text-ink-3">{c.note}</div>
                  </div>
        ...
        <>
          ...
          <Card>
            <div className="mb-2.5 flex items-center gap-2.5">
              <b className="text-[14.5px] font-semibold">Кредитная нагрузка</b>
              <Tag tone={creditLoad / income > 0.3 ? 'gold' : 'brand'}>{pct(creditLoad, income)}% дохода</Tag>
            </div>
            <div className="flex h-4 overflow-hidden rounded-lg bg-track">
              <span className="block h-full" style={{ width: `${(creditLoad / income) * 100}%`, background: 'var(--d2)' }} />
            </div>
            <div className="mt-1 flex justify-between text-[12px] text-ink-3">
              <span>0%</span><span>30% — предел комфорта</span>
            </div>
            <p className="mt-2.5 text-[12.5px] text-ink-2">
              {money(creditLoad)} из {money(income)} дохода. Вместе с жильём обязательные платежи
              занимают {pct(obligationsTotal, income)}%.
            </p>
          </Card>
        </>
      )}
