...
describe('sanitize', () => {
  ...
  it('отчество на «-ична» и двойная фамилия — тоже «Имя Ф.» (критик Блока 1)', () => {
    expect(sanitize('Кенесова Дана Ильинична .')).toBe('Дана К. .')
    expect(sanitize('Иванова-Петрова Анна Сергеевна .')).toBe('Анна И. .')
    expect(sanitize('ИВАНОВА-ПЕТРОВА АННА СЕРГЕЕВНА')).toBe('Анна И.')
  })
